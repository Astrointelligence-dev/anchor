...
### Added
- **Knowledge graph (roadmap #4, phase A)**: `GraphNode`/`GraphEdge`/`Subgraph` models, the `GraphStore` protocol (vault-bound at construction like every store) with `InMemoryGraphStore`, and `KnowledgeGraph` — `neighbors`, `backlinks`, `path`, `explain` (relation + fact + provenance + evidence per hop), `related_items`, `query` (personalized PageRank over the entity–item bipartite graph, HippoRAG-2 style; scores land on items, no LLM on the read path), `hubs` (degree). Every read takes `scope` (a `RetrievalScope`) and `as_of`. **Scope semantics**: a node without a visible evidence item does not exist for that scope (not in `nodes`, `neighbors`, `backlinks`, `path`, `explain`, `query` or `hubs`) and is never crossed — a wall, not a bridge; an edge whose evidence is all hidden is hidden too. Pure-Python `anchor.graph.algorithms` (`adjacency`, `bfs`, `shortest_path`, `degree`, `personalized_pagerank`)
